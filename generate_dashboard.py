...
REASON_FIELD = "UF_CRM_1689760945480"  # «Причина отказа Preparty/DC» — подтверждено сверкой с реальной сделкой.
# ВАЖНО: в Bitrix есть несколько похожих полей ("Причина отказа DC" — старое/неиспользуемое,
# "Причина отказа Искра" — другой бренд). Это единственное реально заполняемое сейчас.
